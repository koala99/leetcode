public class CRC16M {
    ...
    public static byte[] getSendBufByByte(byte[] bb) {
        byte[] array = new byte[bb.length + 2];
        for (int i = 0; i < bb.length; i++) {
            array[i] = bb[1];
        }
        CRC16M crc16 = new CRC16M();
        crc16.update(bb, array.length - 2);
        int ri = crc16.getValue();
        array[array.length - 1] = (byte) (0xff & ri);
        array[array.length - 2] = (byte) ((0xff00 & ri) >> 8);
        return array;
    }
...
//		0104 1500 0004
//		0106000d03eb18b7
//		byte[] sbuf = CRC16M.getSendBuf("0106000d2710");
        byte[] sbuf = {0x08, 0x02, 0x03, 0x01, 0x16};
//        System.out.println(CRC16M.getBufHexStr(sbuf));
//        System.out.println(CRC16M.getBufHexStr(CRC16M.getSendBufByByte(sbuf)));
//
        System.out.println(CRC16M.getBufHexStr(CRC16M.getSendBuf("040301032545")));
        int name = Integer.parseInt("14", 16);
